...
Shader "Unlit/BlinnPhongBump"
{
	...
	fixed4 amb = normalize(_Ambient * _AmbientColor);
	...
	fixed4 NdotL = saturate((dot(normal, lightDir) - 0.2) * _LightColor0);
	fixed4 dif = NdotL * 1 * _Diffuse * _LightColor0;
	...
	Pass
	{
		...
		fixed4 frag(v2f i) : SV_Target{
			...
			float attenuation = 1.0 / (pointLightLength);


			// Compute diffuse ligting
			fixed4 NdotL = saturate((dot(worldPos, pointLightDir) - amb) * _LightColor0);
			//fixed4 dif = NdotL * _Diffuse * _LightColor0 * attenuation;
			float3 diffuseColor = lerp(_LightColor0.rgb * max(0, dot(normal, pointLightDir)), _LightColor0.rgb * max(0, dot(normal, pointLightDir) * amb), sqrt(_WorldSpaceLightPos0.w));

			fixed4 light = fixed4(normalize(diffuseColor) * amb * attenuation, attenuation);
			...
			float NdotH = max(0., dot(normal, halfVector));
			//fixed4 spec = pow(NdotH, _Shininess) * _LightColor0 * _SpecColor;
			fixed4 spec = lerp(pow(NdotH, _Shininess) * _LightColor0 * _SpecColor, pow(NdotH, _Shininess) * _LightColor0 * _SpecColor  * attenuation * amb, sqrt(_WorldSpaceLightPos0.w));
